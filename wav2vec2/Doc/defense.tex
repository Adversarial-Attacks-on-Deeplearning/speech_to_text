...
\newpage
\subsubsection{Methodology}
The MP3 compression defense involves compressing an input audio signal to MP3 format, decompressing it back to the time domain, and transcribing the result using an ASR model. The process is implemented as follows:
...
\State $temp\_file \gets create\_temp\_file(suffix=".mp3")$ \Comment{Create temporary file}
\State $export(audio_{segment}, temp\_file, format="mp3", bitrate=bitrate)$ \Comment{Export as MP3}
\State $compressed_{audio} \gets import\_audio(temp\_file, format="mp3")$ \Comment{Read compressed audio}
\State $delete(temp\_file)$ \Comment{Clean up temporary file}
...
\If{$length(audio_{normalized}) \neq length(x_{audio})$}
    ...
    \Else
        \State $audio_{normalized} \gets pad(audio_{normalized}, length(x_{audio}), mode="constant")$ \Comment{Pad to match input}
    \EndIf
\EndIf
...
\centering
\footnotesize
\caption{MP3 Compression Defense Effectiveness Against PGD Attacks}
...
\subsubsection{Introduction}
Spectral gating is a defense strategy that applies noise reduction to audio signals to remove adversarial perturbations. It uses WebRTC VAD to identify speech and non-speech segments, extracts a noise profile from non-speech regions, and applies spectral subtraction to denoise the audio \cite{oreilly2022voiceblock}. The defense operates in the frequency domain, attenuating noise components while preserving speech quality, making it suitable for streaming applications with low latency.
...
The pseudocode for the spectral gating defense is shown in Algorithm \ref{alg:spectral_gating}.

\begin{algorithm}
\caption{Spectral Gating Defense}
\label{alg:spectral_gating}
\begin{algorithmic}[1]
\Require Audio signal $x$ (float32, [-1,1]), sample rate $sr$, stationary flag $s$, proportion decrease $p$, frequency smoothing $f_{hz}$, time smoothing $t_{ms}$
...
\State $n \gets \text{ExtractNoiseProfile}(x, m, sr)$ \Comment{Get noise from non-speech}
\State $y \gets \text{ReduceNoise}(x, sr, n, s, p, f_{hz}, t_{ms})$ \Comment{Spectral subtraction}
\If{$\text{length}(y) > \text{length}(x)$}
    \State $y \gets y[1:\text{length}(x)]$ \Comment{Trim excess}
\ElsIf{$\text{length}(y) < \text{length}(x)$}
    \State $y \gets \text{Pad}(y, \text{length}(x) - \text{length}(y))$ \Comment{Pad to match}
\EndIf
...
Table \ref{tab:spectral_gating_results} summarizes the results, showing CER, WER, and their reductions ($\Delta$CER, $\Delta$WER) compared to the undefended case for $\epsilon=0.1, \alpha=0.01$. Results for other epsilon values are available in the supplemental material.

\begin{table}[ht]
\caption{Spectral Gating Defense Results for $\epsilon=0.1, \alpha=0.01$}
\label{tab:spectral_gating_results}
\centering
\resizebox{\textwidth}{!}{
\begin{tabular}{lccccr}
\toprule
\textbf{Config} & \textbf{CER (\%)} & \textbf{WER (\%)} & \textbf{$\Delta$CER (\%)} & \textbf{$\Delta$WER (\%)} & \textbf{Samples} \\
\midrule
Original (No Defense) & 37.90 & 61.98 & -- & -- & 100 \\
\texttt{nonstat\_p80\_f500\_t50} & 7.25 & 16.47 & -30.65 & -45.51 & 100 \\
\texttt{stat\_p80\_f500\_t50} & 7.47 & 17.22 & -30.43 & -44.76 & 100 \\
\texttt{nonstat\_p60\_f500\_t50} & 9.88 & 21.39 & -28.02 & -40.59 & 100 \\
\texttt{nonstat\_p100\_f500\_t50} & 2.84 & 7.01 & -35.06 & -54.97 & 100 \\
\texttt{nonstat\_p80\_f200\_t25} & 5.10 & 11.82 & -32.80 & -50.16 & 100 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{table}[ht]
\caption{Input Quantization Defense Results for $\epsilon=0.1, \alpha=0.01$}
...
\midrule
Original (No Defense) & 37.90 & 61.98 & -- & -- & 100 \\
4-bit & 5.56 & 12.91 & -32.34 & -49.07 & 100 \\
8-bit & 14.70 & 29.56 & -23.21 & -32.42 & 100 \\
16-bit & 14.85 & 29.91 & -23.05 & -32.07 & 100 \\
\bottomrule
\end{tabular}
}
\end{table}
