...
\begin{algorithm}
\caption{Fast Gradient Sign Method (FGSM) Attack}
\begin{algorithmic}[1]
...
\For{$i \gets 1$ to $num\_iter$} \Comment{Iterate up to max iterations}
    ...
    \State $loss.\Call{backward}{}$ \Comment{Compute gradients}
    \With{\text{no\_grad}} \Comment{Update perturbation without tracking}
        \With{\text{autocast}(\text{device\_type}=\text{``cuda''}, \text{enabled}=\text{True})} \Comment{Enable mixed precision}
